# NFL Draft Prediction - 特徴量エンジニアリング実行スクリプト
...
print("=== NFL Draft Prediction - 特徴量エンジニアリング実行開始 ===")
...
print(f"\n作成された特徴量総数: {len(feature_cols_fe)}")
...
print(f"\n=== 特徴量エンジニアリング完了 ===")
print(f"- 改良モデルのCVスコア: {np.mean(cv_scores_fe):.5f}")
print(f"- ベースライン: 0.80792")
print(f"- 提出ファイル: improved_submission_with_feature_engineering.csv")
print(f"- 特徴量数: {len(feature_cols_fe)} (元16から大幅増加)") 